(() => {
  ...
  function renderBaharSimpleSettings() {
    ...
    one('.bs-show-qr-caption').checked = settings.showQrCaption !== false;
...
(() => {
  ...
  const defaults = () => ({
    ...
    header:'',
    showHeader:true,
    headerPosition:{xPercent:12.5,yPercent:2.5,widthPercent:75,rotate:0},
    showStamp:true,
  ...
  const mergedSettings = companyEntry => {
    ...
    return Object.assign(base, saved, {
      headerPosition:Object.assign({}, base.headerPosition, saved.headerPosition || {}),
  ...
  function collect(){
    ...
    next.showHeader = overlay.querySelector('.bce-show-header').checked;
  ...
  function attachPreviewDrag(frame){
    attachMovable(frame, '.bahar-contract-header', 'header', 'اسحب الترويسة لتغيير موضعها في العقد فقط');
  ...
  function renderPreview(){
    ...
    frame.onload = () => attachPreviewDrag(frame);
  ...
  function render(){
    ...
    headerCard.dataset.value = draft.header || '';
    root.querySelector('.bce-header-pick').addEventListener('click', () => headerInput.click());
    headerInput.addEventListener('change', () => {
      const file = headerInput.files?.[0];
      if(!file) return;
      const reader = new FileReader();
      reader.onload = () => {
        headerCard.dataset.value = String(reader.result || '');
        headerCard.querySelector('.bce-file-thumb').innerHTML = `<img src="${reader.result}" alt="ترويسة العقد">`;
        root.querySelector('.bce-show-header').checked = true;
        schedulePreview();
      };
      reader.readAsDataURL(file);
    });
    root.querySelector('.bce-header-remove').addEventListener('click', () => {
      headerCard.dataset.value = '';
      headerInput.value = '';
      headerCard.querySelector('.bce-file-thumb').innerHTML = '<span>لا توجد ترويسة — خلفية العقد بيضاء</span>';
      schedulePreview();
    });
...
(() => {
  ...
  const baharSheet = (record, kind, sourceCompany = companyForShipment(record)) => {
    ...
    const body = `
      ...
      <style>.bahar-shipment-sheet .bs-invoice-title{border-bottom:1px solid var(--bs-section-border-color, #000)!important}.bahar-shipment-sheet .operation-qr{padding:.7mm;text-decoration:none}.bahar-shipment-sheet .operation-qr-caption{position:absolute;top:calc(100% + 1mm);left:50%;width:100%;transform:translateX(-50%);text-align:center;color:#17202b;font-family:'IBM Plex Sans Arabic','IBM Plex Sans',Arial,sans-serif;font-size:6.2px;line-height:1.35;font-weight:700;white-space:normal}.bahar-shipment-sheet .operation-qr-caption b,.bahar-shipment-sheet .operation-qr-caption small{display:block;width:100%}.bahar-shipment-sheet .operation-qr-caption small{margin-top:.35mm;font-size:6.2px;font-weight:700;line-height:1.35;direction:ltr;letter-spacing:0}</style>
