...
- بمجرد أن توقّع الإدارة مستند عمليات (ملف التحصيل قيد المراجعة أو معتمد أو مُرسل) يعرض رابط QR — وكذلك معاينة الحزمة المعتمدة في العمليات والملف المدموج في المالية — نفس حزمة العمليات مع استبدال كل مستند له نسخة موقعة بنسخته الموقعة (يُبنى عند الطلب من `api/qr-signed-package.js`، ولا يُعدَّل الملف المخزن). ملف مُعاد للمالية أو للعمليات يعرض الحزمة الأصلية حتى يُوقَّع من جديد. مستندات المالية (خطاب التحصيل، التعهد، الكمبيالة) سرية ولا تدخل رابط QR أبداً.
...
مجلد `pdf-fonts/` يحوي نفس خطوط Google Fonts التي تستخدمها قوالب المستندات
(Tajawal، IBM Plex Sans Arabic، IBM Plex Sans، IBM Plex Mono — رخصة OFL) بصيغة
woff2. عند توليد PDF داخل الحاوية يستبدل الخادم استيراد Google Fonts بهذه الملفات
المضمّنة فلا ينتظر Chromium تنزيل الخطوط عند كل مستند. لو غاب المجلد يعود
الخادم تلقائياً للتحميل من الإنترنت كما كان.
