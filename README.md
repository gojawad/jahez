...
- بعد **القبول النهائي للإدارة** (حالة ملف التحصيل `final_accepted` أو `sent_to_collecting`) يعرض الرابط نفس حزمة العمليات مع استبدال كل مستند عمليات له نسخة موقعة من الإدارة بنسخته الموقعة (يُبنى عند المسح من `api/qr-signed-package.js`). مستندات المالية (خطاب التحصيل، التعهد، الكمبيالة) سرية ولا تدخل رابط QR أبداً.
