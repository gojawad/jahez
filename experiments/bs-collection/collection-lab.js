...
// "THREE HUNDRED ... TWENTY FOUR UAE DIRHAMS AND EIGHTY FILS ONLY". Unknown
// currencies keep the code with a /100 fraction.
function currencyAmountWords(number,currency){
  const code=String(currency||'').trim().toUpperCase(), units=CURRENCY_WORDS[code];
  const minor=Math.round((Number(number)||0)*100), whole=Math.floor(minor/100), fraction=minor%100;
  const wholeWords=amountWords(whole);
  if(!units) return `${wholeWords}${fraction?` AND ${fraction}/100`:''}${code?' '+code:''} ONLY`;
  const [one,many,minorOne,minorMany]=units;
  return `${wholeWords} ${whole===1?one:many}${fraction?` AND ${amountWords(fraction)} ${fraction===1?minorOne:minorMany}`:''} ONLY`;
}
...
function renderDraft(){
  ...
  $('totalsBar').innerHTML=Object.entries(totals).map(([c,n])=>`<div class="total-card"><span>${currencies.length===1?'TOTAL COLLECTION AMOUNT':c+' Total'}</span><strong>${esc(formatMoney(c,n))}</strong><span>${esc(currencyAmountWords(n,c))}</span></div>`).join('')+conversionCard;
...
function renderPreview(){
  const {rows,currencies,consignees,totals}=detected(), s=state.settings, c=currencies[0]||'—', n=totals[c]||0, amount=formatMoney(c,n), words=currencyAmountWords(n,c), drawee=consignees.join(' / ')||'-', refs=rows.map(r=>`${r.invoiceNo||'-'} (${r.invoiceDate||'-'})`).join(', ')||'-';
  let body='';
...
function renderPreview(){
  ...
  const collection=collectionTotal(rows), currency=collection.currency, total=collection.number, amount=formatMoney(currency,total), words=currencyAmountWords(total,currency);
