...
async function main(){
  ...
  try{
    ...
    await page.locator('#permit_proformaNo').fill('');
    await page.locator('#permit_proformaDate').fill('2026-09-20');
    ...
    assert.equal(await page.locator('#permit_proformaNo').inputValue(),saved.data.proformaNo,'generated number survives reopening');
    ...
    await page.locator('#permit_proformaNo').fill('READY-42');
    const manualSave=page.waitForResponse(r=>r.url().includes('/api/import-permit-invoices')&&r.request().method()==='POST');
    await page.locator('#importPermitSaveBtn').click();
    assert.equal((await (await manualSave).json()).record.data.proformaNo,'READY-42','employee can replace the automatic number manually');
    await page.waitForFunction(()=>document.getElementById('importPermitSaveState').textContent.includes('آخر حفظ'));
