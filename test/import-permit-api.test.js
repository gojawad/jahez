...
test('permit API preserves legacy records, decimals, references, authorization and archive pages',async()=>{
  ...
  try{
    ...
    assert.equal(firstAuto.body.record.data.proformaNo,'BSGTP-09-20-2026-1');
    const autoId=firstAuto.body.record.id;
    const repeatAuto=await call('POST',{id:autoId,data:{...autoData,proformaDate:'2026-09-21'}});
    assert.equal(repeatAuto.body.record.data.proformaNo,'BSGTP-09-20-2026-1','editing or blanking an existing number does not renumber it');
    ...
    assert.deepEqual(concurrent.map(r=>r.body.record.data.proformaNo),[2,3,4,5].map(n=>`BSGTP-01-02-2027-${n}`));
    ...
    const afterManual=await call('POST',{data:autoData});assert.equal(afterManual.body.record.data.proformaNo,'BSGTP-09-20-2026-7');
    ...
    assert.equal(restartResponse.body.record.data.proformaNo,'BSGTP-09-20-2026-8','sequence survives server restart');
