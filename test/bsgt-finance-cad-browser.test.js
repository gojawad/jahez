'use strict';
// CAD / paid-in-advance shipments: badged in finance, never mixed with collection shipments,
// their trade file has no collection documents or portal and is sent straight to management.
...
async function main(){
  ...
  try{
    ...
    assert.strictEqual(await page.locator('[data-bsgt-finance-select]').count(),3);
    assert.strictEqual(await page.locator('.bsgt-finance-mode.is-cad').count(),2,'CAD shipments are badged');
    assert.strictEqual(await page.locator('.bsgt-finance-mode.is-collection').count(),1);
    // Mixed CAD + collection selection is refused.
    await page.locator('[data-bsgt-finance-select]').first().check();
    await page.locator('[data-bsgt-finance-select]').last().check();
    assert.strictEqual(await page.locator('#bsgtFinanceCreateBtn').isDisabled(),true,'mixed modes cannot create a file');
    assert.strictEqual(await page.locator('.bsgt-finance-mode-warning').count(),1);
    await page.locator('[data-bsgt-finance-select]').last().uncheck();
    ...
    assert.ok((await page.locator('.bsgt-finance-detail-header').textContent()).includes('CAD'),'detail header shows the CAD mode');
    assert.strictEqual(await page.locator('.bsgt-finance-detail-actions a[href*="tradeFileId"]').count(),0,'no collection portal link for CAD');
    assert.strictEqual(await page.locator('.bsgt-finance-cad-note').count(),1);
    assert.strictEqual(await page.locator('#bsgtTradeSend').isDisabled(),false,'CAD send is not gated on finance originals');
    ...
    assert.deepStrictEqual(lastSendMetadata.documentKinds,[]);assert.strictEqual(lastSendMetadata.collectionMode,'cad');assert.strictEqual(lastSendMetadata.documentSettings.remittingBank.length>0,true);
    console.log('BSGT finance CAD (no-collection) trade file: badges, no mixing, detail and send passed');
    await context.close(); 
