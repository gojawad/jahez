...
async function main(){
  ...
  try{
    ...
    assert.ok(assetState.assets.some(asset=>asset?.includes('bsgt-management.js?v=20260919-cad-1')), 'management JS invalidates the mandatory-signature cached version');
    ...
    await page.locator('#closeDetail').click();
    await page.evaluate(async id=>{await switchView('bsgtWorkspace',{section:'operations'});renderBsgtOperationsDetail(records.find(r=>r.id===id));},shipmentId);
    const quickCorrection=page.locator('#bsgtOperationsReopenCorrection');
    await quickCorrection.waitFor({state:'visible'});
    assert.equal(await quickCorrection.evaluate(button=>Boolean(button.parentElement.querySelector('#bsgtOperationsOpenFull'))),true,'correction is beside the full shipment action');
    await quickCorrection.click();await correctionDialog.waitFor();
    assert.match(await correctionDialog.innerText(),/TC-CORRECTION/);
    await correctionDialog.locator('[data-close]').click();assert.equal(correctionCalls,0,'quick correction confirmation is read only');
    await page.evaluate(id=>{const record=records.find(r=>r.id===id);JahezRevisionWorkflow.decorateOperations({...record,bsgtStage:'sent_to_collecting'});},shipmentId);
    assert.equal(await quickCorrection.count(),0,'dispatched shipments cannot be reopened from quick view');
    await page.evaluate(id=>{currentUser.role='editor';if(currentUser.profile)currentUser.profile.role='editor';syncCurrentPermissionContext();renderBsgtOperationsDetail(records.find(r=>r.id===id));},shipmentId);
    assert.equal(await quickCorrection.count(),0,'employees cannot reopen from quick view');
    await page.evaluate(id=>{currentUser.role='admin';if(currentUser.profile)currentUser.profile.role='admin';syncCurrentPermissionContext();renderBsgtOperationsDetail(records.find(r=>r.id===id));},shipmentId);
    await quickCorrection.waitFor({state:'visible'});
    await page.locator('#bsgtOperationsOpenFull').click();
    await page.locator('#bsgtReopenCorrection').click();
