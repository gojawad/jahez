...
async function main() {
  ...
  try {
    ...
    await check('Bahar sale contract is a compact configurable single A4 sheet', async () => {
      ...
      assert.ok(appHtml.includes("const header = settings.header || '';"));
      assert.ok(appHtml.includes('bahar-contract-header'));
      ...
      assert.ok(appHtml.includes("has-contract-header':''"));
      ...
      assert.ok(companyWizard.includes('ترويسة العقد المستقلة'));
      assert.ok(companyWizard.includes('bce-header-pick'));
      assert.ok(companyWizard.includes("['header','stamp','qr']"));
      assert.ok(appHtml.includes('company-wizard.js?v=20260907-contract-header'));
