...
(async()=>{
  ...
  try {
    ...
    await db.exec(fs.readFileSync(path.join(__dirname,'../supabase/51_bsgt_cad_no_collection.sql'),'utf8'));
    const optionalSql=fs.readFileSync(path.join(__dirname,'../supabase/57_bsgt_optional_stage_signatures.sql'),'utf8');
    const tables=['shipments','trade_collection_files','trade_collection_file_documents','bsgt_operations_revisions','storage.objects'];
    const baseline=[];for(const table of tables)baseline.push((await db.query(`select to_jsonb(t) j from ${table} t order by to_jsonb(t)::text`)).rows);
    await db.exec(optionalSql);await db.exec(optionalSql);
    for(let n=0;n<tables.length;n++)assert.deepEqual((await db.query(`select to_jsonb(t) j from ${tables[n]} t order by to_jsonb(t)::text`)).rows,baseline[n],'57 preserves existing '+tables[n]);
    await db.exec(`create or replace function has_bsgt_workspace_permission(text,boolean) returns boolean language sql as $$select coalesce(current_setting('app.edit',true),'on')='on' and $1=current_setting('app.section',true)$$; set app.section='relations';`);
    let relationPath=`workflow/${tradeFile.id}/2/signed/relations-test.pdf`;
    await db.query("insert into storage.objects values('trade-collection-documents',$1)",[relationPath]);
    const sign=(rev=2,shipment=id,source=`${id}/${revision2}/invoice.pdf`)=>db.query('select register_bsgt_internal_signature($1,$2,$3,$4,$5,$6,$7)',[tradeFile.id,rev,shipment,'invoice',relationPath,source,JSON.stringify([{page:0,x:.1,y:.1,width:.2,height:.1}])]);
    await assert.rejects(sign(),/review changed/i,'dispatched files cannot be signed');
    await db.query("update trade_collection_files set status='final_accepted' where id=$1",[tradeFile.id]);
    await assert.rejects(sign(1),/review changed/i);
    await assert.rejects(sign(2,'10000000-0000-4000-8000-000000000099'),/outside/);
    await assert.rejects(sign(2,id,'other/invoice.pdf'),/original not found/i);
    await db.exec("set app.edit='off'");await assert.rejects(sign(),/permission required/i);await db.exec("set app.edit='on'");
    await sign();
    assert.equal((await db.query("select count(*)::int n from trade_collection_file_documents where trade_file_id=$1 and document_variant='administration_signed' and document_type='invoice' and is_active",[tradeFile.id])).rows[0].n,1);
    assert.ok((await db.query("select 1 from activity_log where text::jsonb->>'action'='relations_signature_saved'")).rows.length);
    await db.exec("set app.section='management'");await assert.rejects(sign(),/review changed/i,'management cannot sign in relations stage');
    await db.query("update trade_collection_files set status='under_management_review' where id=$1",[tradeFile.id]);
    await db.exec("set app.section='relations'");await assert.rejects(sign(),/review changed/i,'relations cannot sign in management stage');
    await db.exec("set app.section='management'");
    relationPath=`workflow/${tradeFile.id}/2/signed/management-test.pdf`;
    await db.query("insert into storage.objects values('trade-collection-documents',$1)",[relationPath]);await sign();
    assert.equal((await db.query('select documents from bsgt_operations_revisions where id=$1',[revision2])).rows[0].documents.find(d=>d.kind==='invoice').path,`${id}/${revision2}/invoice.pdf`,'original path is never overwritten');
    await db.query("update shipments set bsgt_stage='management_review' where id=$1",[id]);
    await db.query('update trade_collection_files set revision_no=3 where id=$1',[tradeFile.id]);
    await assert.rejects(db.query('select final_accept_bsgt_trade_file($1)',[tradeFile.id]),/documents.*incomplete/i,'modern collection still needs originals');
    await db.query("update trade_collection_files set metadata=metadata||'{\"collectionMode\":\"cad\"}'::jsonb where id=$1",[tradeFile.id]);
    await db.query('select final_accept_bsgt_trade_file($1)',[tradeFile.id]);
    const legacyId='40000000-0000-4000-8000-000000000098',legacyShipment='10000000-0000-4000-8000-000000000098';
    await db.query("insert into shipments(id,company_id,status,bsgt_stage,data) values($1,bsgt_company_id(),'sent','management_review','{}')",[legacyShipment]);
    await db.query("insert into trade_collection_files(id,company_id,status,revision_no) values($1,bsgt_company_id(),'under_management_review',1)",[legacyId]);
    await db.query('insert into trade_collection_file_shipments(trade_file_id,shipment_id) values($1,$2)',[legacyId,legacyShipment]);
    await db.query('select final_accept_bsgt_trade_file($1)',[legacyId]);
    ...
    await require('./bsgt-dispatched-correction-sql.cjs')(db,{id,tradeFile,revision2});
    await require('./bsgt-bank-sent-permission-sql.cjs')(db,{id,tradeFile,revision2});
    await require('./bsgt-optional-operations-requirements-sql.cjs')(db);
