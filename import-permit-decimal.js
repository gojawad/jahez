...
(function(root){
  ...
  // Presentation only: keep divide() exact for stored values and calculations.
  function unitPriceDisplay(a,b){
    const x=parts(a),y=parts(b);if(y.n===0n)throw new Error('Division by zero');
    const numerator=x.n*10n**BigInt(y.scale)*100n;
    const denominator=y.n*10n**BigInt(x.scale);
  ...
  const api={decimal,add,multiply,divide,unitPriceDisplay,valid,positive,currency};
